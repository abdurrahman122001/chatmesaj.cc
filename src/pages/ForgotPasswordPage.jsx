...
export default function ForgotPasswordPage({ language = "EN", onChangeLanguage }) {
    ...
    <div className="flex h-screen w-full bg-[#f4f7ff] text-[#111827]">
      <div className="hidden w-[42%] bg-[linear-gradient(160deg,#2563eb_0%,#1d4ed8_45%,#0f172a_100%)] px-10 py-8 text-white lg:flex lg:flex-col lg:justify-between">
        ...
        <div className="max-w-[420px]">
          ...
          <div className="mt-4 text-[15px] leading-[1.6] text-white/75">{copy.forgotSubtitle}</div>
        </div>
      ...
      <div className="flex flex-1 items-center justify-center px-6 py-8">
        <div className="w-full max-w-[440px] rounded-[28px] border border-[#e6ebf4] bg-white p-8 shadow-[0_30px_80px_rgba(37,99,235,0.12)] sm:p-10">
          ...
          <div className="mt-8 text-[34px] font-semibold tracking-[-0.04em] text-[#0f172a]">{copy.forgotTitle}</div>
          <div className="mt-2 text-[15px] text-[#7c8aa5]">{copy.forgotSubtitle}</div>
          ...
          <form onSubmit={handleSubmit} className="mt-8">
            ...
            <button 
              ...
              className={`mt-6 h-[54px] w-full rounded-[14px] text-[17px] font-semibold transition ${email.trim() && !loading ? "bg-[#2563eb] text-white shadow-[0_12px_30px_rgba(37,99,235,0.35)]" : "bg-[#dbeafe] text-[#64748b]"}`}
            >
              {loading ? "Göndərilir..." : copy.forgotSend}
            </button>
          </form>
          
          <div className="mt-6 text-center">
            <a href="/" className="text-[14px] font-medium text-[#2c6cff] hover:underline">{copy.forgotBack}</a>
          </div>
        </div>
      </div>
    </div>
